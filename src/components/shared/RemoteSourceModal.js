...
import { AlertCircle, CheckCircle2, Film, Github, Globe, Image as ImageIcon, LoaderCircle, ShieldCheck, X } from 'lucide-react';
...
export default function RemoteSourceModal({
  ...
  const titleLabel = assetType === 'video' ? 'Remote Video Source' : 'Remote Image Source';
  ...
  const isCompactLayout = typeof window !== 'undefined' && window.innerWidth < 760;
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: getTextOpacity(theme, 0.55), lineHeight: 1.45 }}>
              Enter a public GitHub manifest or HTTPS JSON manifest, preview approved assets, and only connect the source if it passes policy.
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
              <button
                onClick={() => setForm((current) => ({ ...current, provider: 'generic-manifest' }))}
                style={{
                  flex: 1,
                  background: form.provider === 'generic-manifest' ? theme.accent : 'rgba(255,255,255,0.05)',
                  border: 'none',
                  borderRadius: theme.borderRadius,
                  color: form.provider === 'generic-manifest' ? '#fff' : theme.text,
                  padding: '10px 12px',
                  fontSize: 12,
                  fontWeight: 700,
                  cursor: 'pointer',
              ...
              >
                <Globe size={14} /> Manifest
              </button>
              <button
                onClick={() => setForm((current) => ({ ...current, provider: 'github' }))}
                style={{
                  flex: 1,
                  background: form.provider === 'github' ? theme.accent : 'rgba(255,255,255,0.05)',
                  border: 'none',
                  borderRadius: theme.borderRadius,
                  color: form.provider === 'github' ? '#fff' : theme.text,
                  padding: '10px 12px',
                  fontSize: 12,
                  fontWeight: 700,
                  cursor: 'pointer',
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <input
                ...
                placeholder={assetType === 'video' ? 'Calm loop videos' : 'Focus image library'}
            ...
            <div style={{
              ...
              Only approved {assetType === 'video' ? 'videos' : 'images'} appear in the preview. Blocked files stay hidden and are summarized below.
            </div>
          </div>
          ...
          <div style={{ padding: 16, overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 14, flexWrap: 'wrap' }}>
              ...
              >
                {isPreviewing ? <LoaderCircle size={14} className="spin" /> : (assetType === 'video' ? <Film size={14} /> : <ImageIcon size={14} />)}
                ...
                >
                  ...
                  <div style={{ padding: 10 }}>
                    ...
                    <div style={{ fontSize: 10, color: getTextOpacity(theme, 0.48), lineHeight: 1.4 }}>
                      ...
                      {asset.assetType === 'video' && asset.duration ? ` · ${asset.duration}s` : ''}
                    </div>
                  </div>
                </div>
              ))}
            ...
            {preview.status === 'idle' && (
              <div style={{
                ...
                Preview the source to see which {assetType === 'video' ? 'videos' : 'images'} are approved and visible to users.
              </div>
            )}
          </div>
        </div>
